...
public class UpdateTeamMemberHandler : BaseHandler<UpdateTeamMemberCommand, TeamMemberDto>
{
    ...
    public override async Task<TeamMemberDto> HandleRequest(UpdateTeamMemberCommand request, CancellationToken cancellationToken)
    {
        ...
        if (categoryChanged)
        {
            var newCategory = await _repositoryWrapper.TeamCategoriesRepository.GetFirstOrDefaultAsync(
            ...
            if (newCategory == null)
            {
                throw new Exception(ErrorMessagesConstants.NotFound(newCategoryId, typeof(TeamCategory)));
            }
        }
